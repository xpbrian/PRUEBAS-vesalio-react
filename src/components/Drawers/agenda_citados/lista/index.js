...
export default function Index({ selected, lista, handleChanged, handleChangeds }) {
    ...
    useEffect(() => {
        ...
        let dato = (new Date(drawerOpen.item.item.fecha_calendario)).getTime();
        let tzoffset = (new Date()).getTimezoneOffset() * 60000
        let dia = (new Date(Date.now() - tzoffset)).getTime();

        if (parseInt(dato) >= parseInt(dia)) {
            setMostrar(1)
        }
        ...
        <>
            ...
            }
            {
                ...
                >
                    ...
                    <Grid item xs={12}>
                        {
                            mostrar === 1 && <GenerarCita horaSelected={horaSelected} />
                        }
